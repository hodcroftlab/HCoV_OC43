...
def parse_args():
    ...
    parser.add_argument('--out_length', required=True, help='Output file displaying accession and corresponding length')
    parser.add_argument('--range', required=True, help='Specify the region to filter: spike, n, e, m, or whole genome')
...
def main():
    ...
    blast_results = blast_results.sort_values(['evalue', 'length'], ascending=[True, False])
    ...
    blast_results.loc[:, ["qseqid", "diff_length_ref"]].to_csv(args.out_length, index=False, header=["accession", "blast_length"], sep= "\t")
